...
- [Development guidelines](#development-guidelines)
    - [General HellenicCoin Core](#general-helleniccoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
HellenicCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/helleniccoinpay/helleniccoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_helleniccoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_helleniccoin --log_level=test_suite
$ valgrind -v --leak-check=full src/helleniccoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_helleniccoin.coverage/index.html`.
```

**Sanitizers**

HellenicCoin Core can be compiled with various "sanitizers" enabled, which add
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `helleniccoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `helleniccoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of HellenicCoin Core, the wallet was always
